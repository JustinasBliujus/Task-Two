#include "antrastes.h"
void Is_Failo(list<studentas> &grupe, string read_vardas)
{
    ifstream input_file;
    ...
    if (spausdinimo_tipas == 1)
    {
        ...
        {
            cout << setw(15) << it->vardas << setw(15) << it->pavarde;
            cout << setw(10) << setprecision(2) << fixed << galutinis(*it);
            galutinisMediana(*it);
        }
    }
}
...
void galutinisMediana(studentas &temp)
{
    ...
    if (paz_sk % 2 != 0)
    {
        list<int>::iterator it = std::next(temp.paz.begin(), paz_sk / 2);
        mediana = *it;
    }
    else if (paz_sk % 2 == 0)
    {
        list<int>::iterator it = std::next(temp.paz.begin(), paz_sk / 2 - 1);
        int pirmas_paz = *it;
        it = std::next(temp.paz.begin(), paz_sk / 2);
        int antras_paz = *it;
        mediana = (pirmas_paz + antras_paz * 1.0) / 2;
    }
